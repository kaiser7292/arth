...
import { ScreenContainer, Card, LoadingState, PeriodNavigator } from "@/components/ui";
...
export default function MonthlySummaryScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    const topCategories: TopCategory[] = topCats.map((tc) => ({
  ...
  if (!data) {
    return (
      <ScreenContainer padTop={false}>
        <LoadingState message="Loading summary..." icon="bar-chart-outline" />
      </ScreenContainer>
    );
  }

  const spendChange = data.prevMonthSpent > 0
  ...
  return (
    <ScreenContainer padTop={false}>
      <PeriodNavigator mode="month" value={month} onChange={(m) => { setMonth(m); setData(null); }} />

      <ScrollView className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }}>
        {/* Total Spend Card */}
        <Pressable onPress={() => router.push({ pathname: "/budget/transactions" as never, params: { filterMonth: month, title: "Expenses" } })}>
        ...
        <Card className="mx-4 mt-3">
          ...
          <View className="flex-row items-center justify-between">
            <Pressable onPress={() => router.push({ pathname: "/budget/transactions" as never, params: { filterMonth: month, filterAvoidability: "unavoidable", title: "Unavoidable Expenses" } })}>
              <Text className="text-xl font-bold text-foreground">
            ...
            <View
              ...
              style={{
                backgroundColor:
                  data.rightSpendPct >= 70 ? "#22C55E14" :
                  data.rightSpendPct >= 40 ? "#F59E0B14" : "#EF444414",
              }}
            >
              <Ionicons
                name={
                  data.rightSpendPct >= 70 ? "thumbs-up" :
                  data.rightSpendPct >= 40 ? "hand-right" : "thumbs-down"
                }
                size={24}
                color={
