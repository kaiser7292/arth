...
# Stamp the commit SHA into constants/build-info.ts so the bundled JS picks
# it up. Resets to null after build so the working tree stays clean.
cat > "$REPO_ROOT/constants/build-info.ts" <<EOF
// Auto-generated by bin/build-apk.sh on each build.
// Do not edit by hand — the build script overwrites this file.
export const COMMIT_SHA: string | null = "$COMMIT_SHA";
EOF
trap 'cat > "$REPO_ROOT/constants/build-info.ts" <<RESET
// Auto-generated by bin/build-apk.sh on each build.
// Do not edit by hand — the build script overwrites this file.
