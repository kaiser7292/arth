...
try {
    PdfExtractor = requireNativeModule('PdfExtractor');
}
catch {
...
function requireModule() {
    if (!PdfExtractor) {
        throw new Error('PDF extraction is not available. ' +
 ...
 */
export async function getPageCount(filePath, password) {
    const mod = requireModule();
    if (!filePath) {
        throw new Error('File path is required');
    }
 ...
 * @param password - Optional password for encrypted PDFs.
 */
export async function extractTextFromPage(filePath, pageNumber, password) {
    const mod = requireModule();
    if (!filePath) {
        throw new Error('File path is required');
    }
 ...
 * ```
 */
export async function isPasswordProtected(filePath) {
    const mod = requireModule();
    if (!filePath) {
        throw new Error('File path is required');
    }
 ...
 */
export async function extractTextWithInfo(filePath, password) {
    if (!PdfExtractor) {
        return {
            text: '',
            pageCount: 0,
    ...
    try {
        ...
        const passwordRequired = code === 'PASSWORD_REQUIRED' || code === 'INCORRECT_PASSWORD';
        return {
            text: '',
            pageCount: 0,
            success: false,
...
/**
 * Coordinate-based structured row extraction.
 * Returns string[][] — one array per visual row, each element a column string.
 */
export async function extractRows(filePath, password) {
    const mod = requireModule();
    if (!filePath) {
        throw new Error('File path is required');
    }
