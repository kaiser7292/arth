...
| Plugin | File | Effect |
|--------|------|--------|
| `withArmOnly` | `plugins/withArmOnly.js` | arm64-v8a only for APK and AAB via `reactNativeArchitectures` → `ndk.abiFilters` (~120 MB vs ~210 MB); keeps APK named `app-arm64-v8a-release.apk` |
...
7. **Don't add APK `splits { abi }` back.** They conflict with `ndk.abiFilters` in AGP 8+, and while splits are on the React Native gradle plugin skips `abiFilters` — the Play AAB then ships all four ABIs (32-bit phones would install it and crash on arm64-only libs). `withArmOnly` pins `reactNativeArchitectures=arm64-v8a` instead, which covers both APK and AAB.
