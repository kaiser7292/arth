...
export default function SpendingSplitScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [categories, budgets, actuals, classification] = await Promise.all([
        ...
        // v15.2.1: authoritative totals (shared with Monthly Summary so the
        // two screens cannot disagree on unavoidable %).
      ...
      // v15.2.1: Treat -1 (is_right_spend IS NULL) as discretionary for the
      // per-category lists, so expenses without the flag still show up in
      // the UI breakdown. The authoritative percentages come from
      // getSpendClassificationTotals and already bucket NULL into its own
      // "uncategorized" slice, so there's no double counting at the top-
      // level donut/percentage — only at the per-category list level we
      // fold NULL into discretionary so it appears in the breakdown.
      ...
      // Try to get monthly income from yearly plan
      ...
      try {
        ...
        if (plan) {
          monthlyIncome =
            (plan.annual_salary_in_hand + plan.expected_bonus) / 12;
        }
      } catch {
        // No plan — that's fine
      }
      ...
      // v15.2.1: Override the top-level totals / percentages with the
      // shared-helper numbers. The calculateSplit output still drives the
      // per-category lists. Without this override, a month with any
      // uncategorized expense or any null is_right_spend would show a
      // different % here than on Monthly Summary.
      const authoritative: SplitSnapshot = {
        ...
        discretionaryPct:
          classification.discretionaryPct + classification.uncategorizedPct,
      };
  ...
  // handleMonthShift and monthLabel replaced by PeriodNavigator
  ...
  return (
    <ScreenContainer padTop={false}>
      ...
      <ScrollView ref={scrollRef} className="flex-1" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
            ...
            <View className="mx-4 mt-3 mb-2 p-4 rounded-lg bg-card border border-border items-center">
              <DonutChart
                segments={[
                  {
                    label: "Unavoidable",
                    value: split.unavoidableSpent,
                    color: UNAVOIDABLE_COLOR,
                  },
                  {
                    label: "Discretionary",
                    value: split.discretionarySpent,
                ...
                onSegmentPress={(index) => {
                  const y = index === 0
                    ? sectionY.current.unavoidable
                    : sectionY.current.discretionary;
                ...
                {split.maxPossibleSavings !== null && (
                  <View className="items-end">
                    ...
                    <Text className="text-sm font-semibold text-success">
                      {formatAmount(
                        Math.max(Math.round(split.maxPossibleSavings), 0),
                      )}
                    </Text>
                  </View>
                )}
...
function CategoryRow({
  ...
  return (
    <View className="mb-3 last:mb-0">
      ...
      {budget > 0 && (
        <ProgressBar value={pct} color={color} height={6} animated={false} />
      )}
    </View>
  );
}
